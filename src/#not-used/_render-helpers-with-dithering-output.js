...
var _rh = {
  ...
  var _drawToCanvas = function (pixels) {
    ...
    // Define a palette of 256 colors (you can adjust or expand this palette)
    var palette = [
        [0, 0, 0], [255, 255, 255], [255, 0, 0], [0, 255, 0], [0, 0, 255],
    ];
  
    // Convert values to 256 shades of colors with dithering
    for (var y = 0; y < nScreenHeight; y++) {
        for (var x = 0; x < nScreenWidth; x++) {
            var index = y * nScreenWidth + x;
            var color = pixels[index];
            var quantizedColor = findClosestColor(color, palette);
            var quantError = [
            ...
            imageData.data[index * 4] = quantizedColor[0]; // Red
            imageData.data[index * 4 + 1] = quantizedColor[1]; // Green
            imageData.data[index * 4 + 2] = quantizedColor[2]; // Blue
            imageData.data[index * 4 + 3] = 255; // Alpha
  
            // Distribute quantization error to neighboring pixels
            distributeError(pixels, x, y, nScreenWidth, quantError);
        }
    }
  ...
  function findClosestColor(color, palette) {
    ...
    for (var i = 0; i < palette.length; i++) {
        ...
        var distance = calculateDistance(color, paletteColor);
  ...
  // Function to distribute quantization error to neighboring pixels
  function distributeError(pixels, x, y, width, quantError) {
    ...
    for (var i = 0; i < coefficients.length; i++) {
        ...
        if (nx >= 0 && nx < width && ny < pixels.length / width) {
            var index = ny * width + nx;
            var pixel = pixels[index];
            pixels[index] = [
                clamp(pixel[0] + quantError[0] * coeff[0], 0, 255),
                clamp(pixel[1] + quantError[1] * coeff[0], 0, 255),
                clamp(pixel[2] + quantError[2] * coeff[0], 0, 255)
            ];
        }
    }
  }
